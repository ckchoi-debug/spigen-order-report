import base64
import pandas as pd
import snowflake.connector
import sys

SF_ACCOUNT   = "IEPIBKQ-OG88268"
SF_USER      = "TUTOR_SA"
SF_ROLE      = "TUTOR_USER"
SF_WAREHOUSE = "LECTURE_WH"
SF_DATABASE  = "S3"
private_key  = ''

def qry(sql):
    conn = snowflake.connector.connect(
        account=SF_ACCOUNT, user=SF_USER,
        private_key=base64.b64decode(private_key),
        role=SF_ROLE, warehouse=SF_WAREHOUSE,
        database=SF_DATABASE, schema='SAP',
    )
    try:
        cur = conn.cursor()
        cur.execute(sql)
        cols = [d[0] for d in cur.description]
        return pd.DataFrame(cur.fetchall(), columns=cols)
    finally:
        conn.close()

def out(s):
    sys.stdout.buffer.write((s + "\n").encode('utf-8'))

# M0124 FILE_DATE 분포 확인
out("=== M0124 VBRP FILE_DATE 분포 ===")
df1 = qry("""
SELECT p.FILE_DATE, COUNT(*) AS ROW_CNT, SUM(p.NETWR) AS NET_USD
FROM S3.SAP.VBRP p
JOIN S3.SAP.MARA a ON p.MATNR = a.MATNR
WHERE a.MATKL = 'M0124'
GROUP BY p.FILE_DATE
ORDER BY p.FILE_DATE
""")
out(df1.to_string())

# VBRP에서 VBELN별 중복 여부 확인 (M0124 샘플)
out("\n=== M0124 VBRP 중복 패턴 확인 ===")
df2 = qry("""
SELECT p.VBELN, p.POSNR, COUNT(*) AS DUP_CNT, MIN(p.FILE_DATE) AS FIRST_DATE, MAX(p.FILE_DATE) AS LAST_DATE
FROM S3.SAP.VBRP p
JOIN S3.SAP.MARA a ON p.MATNR = a.MATNR
WHERE a.MATKL = 'M0124'
GROUP BY p.VBELN, p.POSNR
HAVING COUNT(*) > 1
ORDER BY DUP_CNT DESC
LIMIT 10
""")
out(df2.to_string())

# 원래 분석과 동일한 방식: VBRK FILE_DATE로 VBRP join (MAX FILE_DATE 없이 전체)
out("\n=== M0124 전체 기간 (FILE_DATE 조건 없이, VBRP+VBRK 직접 join) ===")
df3 = qry("""
SELECT
    TO_CHAR(DATE_TRUNC('month', h.FKDAT), 'YYYY-MM') AS YEARMONTH,
    SUM(p.FKIMG) AS QTY,
    SUM(p.NETWR) AS NET_USD
FROM S3.SAP.VBRP p
JOIN S3.SAP.VBRK h ON p.VBELN = h.VBELN AND p.FILE_DATE = h.FILE_DATE
JOIN S3.SAP.MARA a ON p.MATNR = a.MATNR
WHERE h.FKART NOT IN ('S1','S2','RE','RK')
  AND p.NETWR > 0
  AND h.WAERK = 'USD'
  AND h.FKDAT >= '2025-01-01'
  AND a.MATKL = 'M0124'
GROUP BY YEARMONTH
ORDER BY YEARMONTH
""")
out(df3.to_string())
